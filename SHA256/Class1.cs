...
// берём дробную часть и умножить на 2^32
...
string sha256(string message)
{
    ...
    cout << "Начальные значения H (из квадратных корней):\n";
    for (int i = 0; i < 8; i++)
    {
    ...
    cout << "Константы K (из кубических корней, показываем первые 8):\n";
    for (int i = 0; i < 64; i++)
    {
        while (!isPrime(number)) number++;
    ...
    cout << "Длина сообщения после padding: " << message.length() << " байт\n\n";
    ...
    for (int i = 0; i < 16; i++)
    {
        W[i] = ((uint8_t)message[i * 4] << 24) |
               ((uint8_t)message[i * 4 + 1] << 16) |
               ((uint8_t)message[i * 4 + 2] << 8) |
               ((uint8_t)message[i * 4 + 3]);
    }
